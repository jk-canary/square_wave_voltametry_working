...
//uint32_t hwError = 0;
//uint8_t uartMemory[UART_MEMORY_SIZE];
...
//void handleCommand(Command_Message *msg);
...
int main(void)
{
  ...
  /*Command_Message recvMsg;
  while (1) 
	{
    if (adi_uart_Read(hUart, &recvMsg, sizeof(recvMsg),false, &hwError) == ADI_UART_SUCCESS) 
		{
      handleCommand(&recvMsg);
    }
  }*/
}
...
void handleCommand(Command_Message *msg) {
  ...
  if (msg->type == CMD_START_STOP) {
    //printf("Received CMD_START_STOP: start=%d, stop=%d\n", msg->Start_Stop_mode.start, msg->Start_Stop_mode.stop);

    if (msg->Start_Stop_mode.start == true) {
			 AD5940_Main();
      // Send data back as RECEIVE_Message
      //RECEIVE_Message recvMsg;

      //recvMsg.type = CMD_data;
      //recvMsg.Data_mode.cur_val = 123; // Example data value

      //adi_uart_SubmitTxBuffer(hUart, &recvMsg, sizeof(recvMsg), false);
      //printf("Sent RECEIVE_Message: CMD_data with cur_val=123\n");
    }
  }
	   // msg->Start_Stop_mode.start = false;
    //msg->Start_Stop_mode.stop = 0;
}
*/
