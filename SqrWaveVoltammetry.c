  ...
  }else
  {
	...
	if(!AppSWVCfg.bRampOneDir)
	{
		if(AppSWVCfg.bSqrWaveHiLevel)
		{
			if(AppSWVCfg.bDACCodeInc)
				{
					AppSWVCfg.CurrRampCode =  -1*(AppSWVCfg.currINc  - (AppSWVCfg.DACCodePerStep)); //- AppSWVCfg.DACCodePerRamp);
					//printf("\n \n %f", AppSWVCfg.DACCodePerRamp);
					if((AppSWVCfg.CurrStepPos <= AppSWVCfg.StepNumber/2))
					AppSWVCfg.currINc += AppSWVCfg.DACCodePerRamp;
					else
						AppSWVCfg.currINc -= AppSWVCfg.DACCodePerRamp;
			...
			else
			{
				...
				AppSWVCfg.CurrRampCode =  -1*(AppSWVCfg.currINc  + (AppSWVCfg.DACCodePerStep)) ;
				AppSWVCfg.currINc -= AppSWVCfg.DACCodePerRamp;
			...
			if(AppSWVCfg.bDACCodeInc)
				{
					...
					if(AppSWVCfg.CurrStepPos < AppSWVCfg.StepNumber)
					AppSWVCfg.currINc += AppSWVCfg.DACCodePerRamp;
					else if(AppSWVCfg.CurrStepPos == AppSWVCfg.StepNumber)
						AppSWVCfg.currINc = AppSWVCfg.RampStartVolt;
		//			else
		//				AppSWVCfg.currINc -= AppSWVCfg.DACCodePerRamp;
  ...
  if(IntFlag & AFEINTSRC_ENDSEQ)
  {
    ...
    *pCount = FifoCnt;
    AppSWVCtrl(APPCTRL_STOPNOW, 0);    /* Stop the Wakeup Timer. */
		...
		AppSWVCfg.bTestFinished = bTRUE;
		AppSWVCfg.RampState = SWV_STATE0;
    AppSWVCfg.bFirstDACSeq = bTRUE;
    AppSWVCfg.bDACCodeInc = bTRUE;
		AppSWVCfg.bSqrWaveHiLevel = bFALSE;
		
		AppSWVCfg.currINc =  AppSWVCfg.RampStartVolt/DAC12BITVOLT_1LSB;
